# 👋 Hi, I'm Naresh Chaudhary

### 🧠 Applied AI & Agentic Systems Engineer

I design and build **production-grade AI systems** using LLMs, Retrieval-Augmented Generation (RAG), and multi-agent architectures.

My work focuses on building **end-to-end AI products** — from backend systems and retrieval pipelines to voice interfaces and mobile applications.

---

### 🚀 LIVE AI APPLICATIONS

| Project | Description | Live Link |
| :--- | :--- | :--- |
| **AI Career Agent** | Resume Analysis & Recommendation System using **RAG**  | [View Live](https://career-agent-ooiz.onrender.com/)  |
| **Luma** | Voice-Based AI Movie Assistant with **LLM + Voice/Speech Pipeline**  | [View Live](https://lumamovieagent-2roo.onrender.com))  |
| **DrapeAI** | **AI Fashion Recommendation System** for personalized styling | [View Live](https://drape-ai.onrender.com/)  |
| **Ultra Doc Intelligence Agent** | Advanced document processing with **Optimized RAG + LLM** | [View Live](https://ultra-doc-intelligence-mwcs.onrender.com/) |

---

## 🏆 Featured Work

### 🚀 AgentsMarketplace *(Flagship — In Progress)*

A voice-first **multi-agent AI platform** where each user gets a private AI “brain”

* Multi-agent orchestration (LangGraph)
* Per-user RAG (FAISS + embeddings)
* Voice pipeline (STT → LLM → TTS)
* Redis-based memory system

---

### 👔 Lin.O — AI Recruitment Platform

AI-powered hiring system with **semantic matching + AI interviews + evaluation**

* Resume parsing + FAISS-based matching
* AI interview system (LLM + voice)
* Candidate scoring & feedback

---

### 👗 DrapeAI — AI Personal Styling App *(Mobile + AI Product)*

AI-powered fashion recommendation system built as a **cross-platform mobile app**

* AI wardrobe digitization (GPT-4o Vision)
* Hybrid recommendation engine (rules + AI scoring)
* Weather-aware outfit selection
* Built with **Flutter + FastAPI + MongoDB + Redis**

👉 Focused on solving a real consumer problem: *“What should I wear today?”*

---

### 🎬 Multilingual Voice AI Movie Assistant

Voice-enabled AI assistant with **RAG + fine-tuned LLM (QLoRA)**

* Semantic retrieval (FAISS)
* Voice interaction pipeline
* Domain guardrails for hallucination control

---

### 🧠 AI Career Agent

RAG-based system for **resume analysis, skill-gap detection, and career guidance**

---

## ⚙️ Tech Stack

**AI / LLM Systems**
RAG · LangChain · LangGraph · FAISS · ChromaDB · OpenAI · Mistral · Sentence Transformers

**Backend Engineering**
Python · FastAPI · Django · REST APIs · JWT Auth

**Mobile & Frontend**
Flutter · Riverpod · Tailwind

**Systems & Infrastructure**
Docker · Redis · PostgreSQL · MongoDB · AWS

---

## 🧠 Engineering Philosophy

I build AI systems with a focus on:

* Retrieval over hallucination
* Systems over scripts
* Real-world usability over demos
* Product-first thinking (not just models)

---

## 🎯 Background

I bring **15+ years of recruitment experience**, which helps me design AI systems grounded in real-world decision-making.

Currently pursuing **M.Tech in AI/ML (BITS Pilani)**.

---

## 📫 Connect With Me

📧 [n26code@gmail.com](mailto:n26code@gmail.com)
🔗 https://github.com/n2coder

---

⭐ If you're building something interesting in AI, let’s connect.
